...
- `rl9_cfg` — 앱 설정
- `rl9_draft` — 저장 전 입력 임시 보관 `{ "YYYY-MM-DD": log 항목과 같은 모양 }`. 아래 '쓰다 만 기록' 참고
...
### 쓰다 만 기록 (저장 전 임시 보관, `rl9_draft`)

기록 화면에서 저장을 누르기 전에 탭을 옮기거나 앱이 꺼져도 입력이 남도록, 값이 바뀔 때마다
날짜별로 `rl9_draft` 에 보관한다 (`draftSave()`). 그 날짜를 다시 열면 `rEntry()` 가 저장된
`logs` 대신 보관된 값을 올리고 맨 위에 안내 띠 + **버리기** 버튼을 보인다.

- 비교 기준은 **열었을 때 화면에 보인 값**(`formFromLog()`, 시작 km 자동 채움 포함). 열기만 하고
  나가면 보관되지 않고, 바꿨다가 원래대로 돌려놓으면 보관이 풀린다.
- 저장(`saveE`)·삭제(`delE`)·버리기(`draftDiscard`)에서 지운다.
- 달력은 헤더 아래 줄에 **이어쓰기** 링크(가장 최근 날짜)와 해당 칸 테두리(`.cc.draft`)로 알린다.
- 사진은 고르는 즉시 IndexedDB 에 들어가므로 대상이 아니다. 백업에도 포함하지 않는다.
...
| 화면 | 함수 | 진입 | 설명 |
|----|------|------|------|
| 📅 달력 | `rCal()` | 탭바 | 월별 캘린더. 셀에 바리수/폐수/오티/주유/톨비/사진 뱃지 표시. **쉬는 날(휴무·정비·기타)은 메모 내용을 칸에 표시**(왜 쉬었는지). 화면 꽉차게 채움. 좌우 스와이프로 월 이동(애니메이션). |
...
### 칸에 찍히는 한 줄 (`.dsub`) 우선순위

`메모(쉬는 날만)` > `기념일` > `공휴일` > `절기`. 한 줄만 나온다.

**메모는 `st` 가 `vacation`/`repair`/`other` 인 날에만 찍는다.** 근무일은 바리수·주유·오티
뱃지가 칸을 이미 채워서 메모를 넣으면 뱃지가 잘린다. 쉬는 날은 뱃지가 없거나 하나뿐이라
자리가 남고, 정작 "왜 쉬었는지"는 달력에서 확인할 방법이 그것뿐이다.
긴 메모는 `text-overflow:ellipsis` 로 잘리므로 짧게 쓰면 그대로 보인다.

**칸 높이는 못 늘린다.** 375×812 6주 달 기준 칸이 84px인데, 늘릴 여지는 하단 요약바
61px 뿐이라 6줄로 나누면 칸당 +11px 이 전부다. 뱃지가 3개면 칸 안쪽 폭 49px 를 넘겨
(주유+OT+톨비 = 73px) 두 줄로 접히면서 미니줄 하나가 18px 대신 36px 을 먹는 게 진짜
병목이다. 뱃지를 8px + 줄임말로 줄이면 한 줄에 들어가지만, 2026-07-27 에 가독성 때문에
텍스트를 1.25배 키운 것과 정반대라 하지 않았다.
