...
레미콘(콘크리트 믹서트럭) 운전기사를 위한 개인용 운행일지 PWA. 순수 HTML/CSS/JS로 작성되었으며 외부 라이브러리 의존성이 없고, 기록·설정은 브라우저 `localStorage`에, 운행일지 사진은 `IndexedDB`에 저장된다.
...
사진은 `logs`에 넣지 않는다. 별도 IndexedDB에 원본 파일 그대로 들어간다.

| 항목 | 값 |
|---|---|
| DB 이름 | `rl_photo` |
| 오브젝트 스토어 | `p` |
| 키 | 날짜 문자열 (`"2026-08-17"`) — `logs`의 키와 동일 |
| 값 | 카메라/갤러리에서 받은 원본 `File`(Blob). 압축·base64 변환 없음 |

하루 1장. 키가 날짜에서 파생되므로 `logs`에 사진 ID를 들고 있을 필요가 없다.
백업(`doBackup()`)은 `logs`·`cfg`만 직렬화하므로 **사진은 백업에 포함되지 않는다.**
...
하단 탭바에는 4개 탭(달력/월말결산/연말결산/설정)만 노출되며, **기록 화면은 달력에서 날짜를 탭해 진입**한다. `tab` 전역변수 + `go(tabName)` + `render()`로 전환.
...
| 📊 월말결산 | `rStats()` | 탭바 | 월말결산. 기본급/오티/2시간초과/톨비/정비 합산. 운행 km는 **이달 누적만** 표시(전체 누적은 미표시). **유류는 정산에서 제외**(월별 리셋, 현황만 표시). |
...
- **사진**: 촬영·갤러리 선택 즉시 IndexedDB에 원본 Blob으로 저장(저장 버튼과 무관). 뷰포트가 핀치 확대를 막으므로, 사진을 탭하면 **오버레이에 원본 크기로 얹고 쓸어서** 읽는다. **`window.open` 으로 blob URL을 열지 않는다** (아래 iOS 항목).
...
- **사진 확대(`phZoom()`)도 같은 이유로 `window.open` 을 쓰지 않는다.** 오버레이에
  원본 크기로 얹고 쓸어서 본다. 사진 blob URL 은 `render()` 마다 revoke 되므로
  화면을 이동시켰다면 정확히 같은 사고가 난다.
...
- 운행일지 사진은 백업(.json)에 포함되지 않는다 → 폰을 바꾸면 사진은 옮겨지지 않는다. 기록·설정만 이전된다.
